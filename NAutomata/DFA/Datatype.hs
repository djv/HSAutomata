...
data MapDFA stateLabel a = MapDFA { transitionMap :: Gr stateLabel a
                                  , startKey :: stateLabel
...
mapDFAToDFA (MapDFA t s a) = DFA (transitionMapToFunction t) s (`Set.member` a)
    where transitionMapToFunction g node edge = listToMaybe $ map fst $ filter ((==edge) . snd) $ lsuc g node

-- | Set of states in a DFA.
states (MapDFA m _ _) = nodes m
